...
const (
	...
	DefaultMinRelayTxFee = ppcutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee ppcutil.Amount) int64 {
	...
	if minFee < 0 || minFee > ppcutil.MaxSatoshi {
		minFee = ppcutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *ppcutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee ppcutil.Amount) bool {
...
func checkTransactionStandard(tx *ppcutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee ppcutil.Amount,
...
func GetTxVirtualSize(tx *ppcutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
