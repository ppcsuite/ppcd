...
// a slice of them.
func loadBlocks(t *testing.T, dataFile string, network wire.BitcoinNet) ([]*ppcutil.Block, error) {
	...
	blocks := make([]*ppcutil.Block, 0, 256)
	genesis := ppcutil.NewBlock(chaincfg.MainNetParams.GenesisBlock)
	...
	for height := 1; ; height++ {
		...
		block, err := ppcutil.NewBlockFromBytes(blockBytes)
		if err != nil {
