...
// specified address.
func PayToAddrScript(addr ppcutil.Address) ([]byte, error) {
	...
	case *ppcutil.AddressWitnessPubKeyHash:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
		return payToWitnessPubKeyHashScript(addr.ScriptAddress())
	case *ppcutil.AddressWitnessScriptHash:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
...
func MultiSigScript(pubkeys []*ppcutil.AddressPubKey, nrequired int) ([]byte, error) {
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *chaincfg.Params) (ScriptClass, []ppcutil.Address, int, error) {
	var addrs []ppcutil.Address
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := ppcutil.NewAddressPubKeyHash(pops[2].data,
			chainParams)
		if err == nil {
