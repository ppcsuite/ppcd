...
// newAddressPubKey returns a new ppcutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) ppcutil.Address {
	addr, err := ppcutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new ppcutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) ppcutil.Address {
	addr, err := ppcutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new ppcutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) ppcutil.Address {
	addr, err := ppcutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the ppcutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the ppcutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := ppcutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := ppcutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := ppcutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := ppcutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := ppcutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*ppcutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*ppcutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*ppcutil.AddressPubKey)(nil), "", errUnsupportedAddress},
